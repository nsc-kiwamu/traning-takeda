 ...
 */ 
  const sample1 = ["a","b","c"] // 要素数3つ
  const sample2 = ["d","e"] // 要素数2つ
  makeMap(sample1,sample2)
 ...
 */
  const number = 10
  const backnumber = praMap8(number)
 ...
 　　引数:string[]、戻り値:number
     引数で受け取った配列の要素数を返却する
 */
     const sampleString = ['赤','青','黄色']
     const backString = praMap9(sampleString)
     console.log(`引数で受け取った配列の要素数${backString}が返却されました\r\n`)
